const CONFIG = {
  ...
  RECAPTCHA_SECRET_KEY: '6LchqSwtAAAAAOc2s6dk41ly-4vjaLChtL7_-BYe',
...
function doPost(e) {
  try {
    ...
    const captchaOk = verificarRecaptcha(data.recaptchaToken);
    if (!captchaOk) {
      return respuestaOk({ status: 'error', message: 'Verificación de seguridad fallida. Intenta nuevamente.' });
    }
...
function verificarRecaptcha(token) {
  // Si no llegó token (script bloqueado, etc.) se deja pasar para no romper el flujo del cliente
  ...
  try {
    ...
    Logger.log('reCAPTCHA score: ' + result.score);

    if (!result.success) return false;
    if (typeof result.score === 'number' && result.score < CONFIG.RECAPTCHA_MIN_SCORE) return false;

    return true;
  } catch (err) {
    Logger.log('Error verificando reCAPTCHA: ' + err.message);
    return true; // si falla la verificación por error de red, no bloquea el envío
  }
}
